...
public class CrossSiteScriptingLesson5a implements AssignmentEndpoint {
  ...
  public AttackResult completed(
    ...
    // The attack is judged on what the cart actually renders, which is entity encoded.
    if (XSS_PATTERN.test(creditCard)) {
      userSessionData.setValue("xss-reflected-5a-complete", "true");
      if (creditCard.toLowerCase().contains("console.log")) {
        return success(this)
